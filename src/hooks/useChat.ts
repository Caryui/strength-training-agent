...
import { Message, ToolCall, PermissionRequest, PermissionMode, Session, CustomAgent, ContentBlock, UserProfile, TrainingLogEntry } from '../types';
import { buildProfileContext, isProfileFilled } from '../utils/profileContext';
...
interface UseChatOptions {
  ...
  /** 用户个人档案（注入 systemPrompt） */
  userProfile?: UserProfile | null;
  /** 训练记录（注入 systemPrompt，用于 RPE 调节） */
...
export function useChat(options: UseChatOptions) {
  ...
  const sendMessage = useCallback(async (
    ...
    // 注入个人档案 + 训练记录，使 Agent 始终基于用户真实数据出方案
    if (userProfile && isProfileFilled(userProfile)) {
      systemPrompt = `${systemPrompt}\n\n${buildProfileContext(userProfile, trainingLog || [])}`;
    }
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          model: selectedModel,
          systemPrompt,
          userProfile: userProfile && isProfileFilled(userProfile) ? userProfile : undefined,
          trainingLog: trainingLog && trainingLog.length > 0 ? trainingLog : undefined,
          cwd: currentCwd,
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (data.type === 'init') {
                  ...
                  // 后端返回的明确错误（如 SDK 鉴权失败），必须展示给用户
                  const errMsg = data.message || '生成失败，请重试';
                  console.error('[Chat] 后端错误:', errMsg);
                  setSessions(prev => prev.map(s => {
                    if (s.id === realSessionId) {
                      return {
                        ...s,
                        messages: s.messages.map(m =>
                          m.id === realAssistantMessageId
                            ? { ...m, content: `⚠️ ${errMsg}`, isStreaming: false, isError: true }
                            : m
                        )
                      };
                    }
  ...
  }, [currentSession, currentSessionId, selectedModel, getAgent, updateSessionModel, setCurrentSessionId, setSessions, userProfile, trainingLog, isLoading]);
